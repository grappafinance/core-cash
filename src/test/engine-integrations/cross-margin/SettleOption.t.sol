...
contract TestSettleCoveredCall_CM is CrossMarginFixture {
    ...
    function testCannotGetPayoutForPhysical() public {
        uint256 physical = getTokenId(SettlementType.PHYSICAL, TokenType.PUT, pidUsdcCollat, expiry, strike, 0);

        vm.expectRevert(GP_InvalidSettlement.selector);

        grappa.settleOption(alice, physical, amount);
    }
    ...
    function testSellerCanClearOnlyExpiredOptions() public {
        ...
        uint256 tokenId2 = getTokenId(SettlementType.CASH, TokenType.CALL, pidEthCollat, expiry + 1 days, strike, 0);
        ActionArgs[] memory actions = new ActionArgs[](2);
        actions[0] = createAddCollateralAction(wethId, address(this), depositAmount);
    ...
    function testSellerCanClearMultipleExpiredOptions() public {
        vm.warp(expiry - 10 days);
        uint256 tokenId2 = getTokenId(SettlementType.CASH, TokenType.CALL, pidEthCollat, expiry, strike + (1 * UNIT), 0);
...
contract TestSettleCollateralizedPut_CM is CrossMarginFixture {
    ...
    function testSellerCanClearOnlyExpiredOptions() public {
        uint256 tokenId2 = getTokenId(SettlementType.CASH, TokenType.PUT, pidUsdcCollat, expiry + 1 days, strike, 0);
        ActionArgs[] memory actions = new ActionArgs[](2);
        actions[0] = createAddCollateralAction(usdcId, address(this), depositAmount);
        actions[1] = createMintAction(tokenId2, alice, amount);
        engine.execute(address(this), actions);
...
contract TestLongShortSettlement is CrossMarginFixture {
    ...
    function setUp() public {
        ...
        selfTokenId = getTokenId(SettlementType.CASH, TokenType.PUT, pidUsdcCollat, expiry, selfStrike, 0);
        aliceTokenId = getTokenId(SettlementType.CASH, TokenType.PUT, pidUsdcCollat, expiry, aliceStrike, 0);
