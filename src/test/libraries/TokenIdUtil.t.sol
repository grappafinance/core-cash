...
contract TokenIdUtilTester {
    function getTokenId(
        ...
        uint256 result = TokenIdUtil.getTokenId(tokenType, settlementType, productId, expiry, longStrike, shortStrike);
        return result;
    }
...
contract TokenIdLibTest is Test {
    ...
    function testIsExpired() public {
        ...
        uint256 tokenId = tester.getTokenId(TokenType.PUT, SettlementType.CASH, 0, expiry, 0, 0);
        assertEq(tester.isExpired(tokenId), false);

        uint64 expiry2 = uint64(block.timestamp - 1);
        uint256 tokenId2 = tester.getTokenId(TokenType.PUT, SettlementType.CASH, 0, expiry2, 0, 0);
