...
contract Settlement is AssetRegistry {
    ...
    function getOptionPayout(uint256 _tokenId, uint256 _amount) public view returns (address, uint256 payout) {
        ...
        if (tokenType == TokenType.CALL) {
            cashValue = SimpleMarginLib.getCallCashValue(expiryPrice, longStrike);
        } else if (tokenType == TokenType.CALL_SPREAD) {
            cashValue = SimpleMarginLib.getCashValueCallDebitSpread(expiryPrice, longStrike, shortStrike);
        } else if (tokenType == TokenType.PUT) {
            cashValue = SimpleMarginLib.getPutCashValue(expiryPrice, longStrike);
        } else if (tokenType == TokenType.PUT_SPREAD) {
            cashValue = SimpleMarginLib.getCashValuePutDebitSpread(expiryPrice, longStrike, shortStrike);
        }
